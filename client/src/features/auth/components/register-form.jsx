...
export default function RegisterForm() {
  ...
  const validateForm = () => {
    ...
    // 4. Password Length (Must match backend requirement)
    if (formData.password.length < 8) {
      return { code: 'weak_password', message: 'Password must be at least 8 characters long.' };
    }

    // 5. Phone Validation (Simple check, backend requires 7-20 chars)
    if (formData.phone.length < 7) {
      return { code: 'invalid_phone', message: 'Phone number is too short.' };
    }
